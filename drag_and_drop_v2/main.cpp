...
HRESULT InitD3D(HWND hWnd)
{
    ...
    // Получаем текущий режим экрана для полноэкранного режима
    D3DDISPLAYMODE d3ddm;
    if (FAILED(g_pD3D->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &d3ddm)))
        return E_FAIL;

    D3DPRESENT_PARAMETERS d3dpp = {};
    d3dpp.Windowed = FALSE; // Важно: полноэкранный режим
    d3dpp.SwapEffect = D3DSWAPEFFECT_DISCARD;
    d3dpp.BackBufferFormat = d3ddm.Format; // Формат текущего экрана
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow)
{
    WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0, 0,
                      ...
                      "DX9DragDropFullScreenClass", NULL };
    RegisterClassEx(&wc);

    // Создаём полноэкранное окно (без рамок)
    HWND hWnd = CreateWindow(wc.lpszClassName, "DirectX9 Drag and Drop Fullscreen",
                             WS_POPUP, 0, 0,
